addHE<-function(range,color,ird=T,axis3=T) {
		# heinrich timing #1
	...
	# heinrich timing #2
	heinrich1=c(11500,14500,22000,27000,36000,44500,52500,57500,70000,74000,82500) # all read from Dr. Rial's ArcticWarmDomec.ai file
	heinrich2=c(12500,15500,22500,29000,37500,46000,54000,59000,72000,75500,84000)

	heinrich_ran=cbind(heinrich1,heinrich2)
	heinrich=apply(heinrich_ran,1,mean)
	hname=c('H0','H1','H2','H3','H4','H5','H5a','H6','H7?','H8?','H9?')
	...
	if (axis3) axis(3,at=c(heinrich,irds),label=c(hname,irdn))
